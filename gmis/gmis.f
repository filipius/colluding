...
c
c     ------------------------------------------------------------------
      integer  biter0,i,k,maxtup,nq,tpl

c     ------------------------------------------------------------------
c     Check if parameters are set correctly.
c     ------------------------------------------------------------------
      call chkerr(alpha,beta,errcnd,look4,lstype,lswhen,maxa,maxitr,
     +             maxn,maxtd,nlowdg,narcs0,nnode0,seed,tupdim)
...
c          -------------------------------------------------------------
           call mkrgrf(adjn,adjn0,deg,deg0,fwdmap,iniad,iniad0,invmap,
...
c
c     ------------------------------------------------------------------
      integer   cutoff,dv,freedm,i,index,iv,k,maxfdm,minfdm,nfdmq,tpl
c     ------------------------------------------------------------------
...
c     ------------------------------------------------------------------
      maxfdm=-freedm
      cutoff=minfdm+beta*(maxfdm-minfdm)
c     ------------------------------------------------------------------
...
c
c     ------------------------------------------------------------------
      integer  freedm,i,j,k,ptr,sumadj
c     ------------------------------------------------------------------
c
c     Passed input arrays:
c
...
c     ------------------------------------------------------------------
      do 30 i=1,tupdim-1
	   do 20 j=i+1,tupdim
                if (incdij((vtup(i)-1)*nnode0+vtup(j)).eq.1) return
20         continue
30    continue
c     ------------------------------------------------------------------
...
c     ------------------------------------------------------------------
      ...
      subroutine mkrgrf(adjn,adjn0,deg,deg0,fwdmap,iniad,iniad0,invmap,
     +                 max2a,max2n2,maxn,maxtd,narcs,nnode,nnode0,ptrn,
...
c          deg0   - degrees of original graph
...
c
c     ------------------------------------------------------------------
      integer adjn0(max2a),deg0(maxn),iniad0(maxn),ptrn0(max2a),
     +        vtup(maxtd)
c     ------------------------------------------------------------------
c
c     Passed output arrays:
...
c          ph1sol - size of largest set found in phase 1 (saved)
c
c     ------------------------------------------------------------------
      double precision  avgph1
      integer nbkset,ph1sol
c     ------------------------------------------------------------------
...
c          -------------------------------------------------------------
c          Save value of phase 1 solution and update average of phase 1
c          solutions.
c          -------------------------------------------------------------
	   ph1sol=nbkset
...
c     ------------------------------------------------------------------
      ...
      subroutine chkerr(alpha,beta,errcnd,look4,lstype,lswhen,maxa,
     +                  maxitr,maxn,maxtd,nlowdg,narcs0,nnode0,seed,
     +                  tupdim)
c     ------------------------------------------------------------------
c
...
c     ------------------------------------------------------------------
      real     alpha,beta
      integer  look4,lstype,lswhen,maxa,maxitr,maxn,
     +         maxtd,narcs0,nlowdg,nnode0,seed,tupdim
c     ------------------------------------------------------------------
c
c     Passed input/output scalars:
c
...
c          ptr    - ptr
c          ptrv   - ptrv
c          randp  - pseudo random generator function
c          u      - vertex
c          v      - vertex
...
c          -------------------------------------------------------------
	   cutoff=mindeg + alpha*(maxdeg-mindeg)
c          -------------------------------------------------------------
...
c          Function randp returns a seed between 1 and 2147483647.
c          -------------------------------------------------------------
           xrand=randp(seed)
...
c
c     ------------------------------------------------------------------
      integer bksetk,i,index,invu1,invu2,k,k1,k2,nvset,ptr,u1,
     +        u2,v1,w
c     ------------------------------------------------------------------
...
c          (bkset), check if there exist pairwise nonadjacent vertices 
c          (u1,u2,u3) such that u1, u2, and and u3 are not adjacent to 
c          bkset-v1-v2.
c          -------------------------------------------------------------
	   do 110 i=1,nbkset-1
	        v1=bkset(i)
